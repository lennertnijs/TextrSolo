...
public class FileBufferTest {
    ...
    public void testEqualsAndHashCode(){
        Point point = Point.create(1,1);
        FileBuffer buffer1 = FileBuffer.builder().id(0).fileId(0).bufferText("text").insertionPosition(point).state(BufferState.CLEAN).build();
        FileBuffer buffer2 = FileBuffer.builder().id(1).fileId(0).bufferText("text").insertionPosition(point).state(BufferState.CLEAN).build();
        FileBuffer buffer3 = FileBuffer.builder().id(0).fileId(0).bufferText("text").insertionPosition(point).state(BufferState.CLEAN).build();
        Assertions.assertAll(
